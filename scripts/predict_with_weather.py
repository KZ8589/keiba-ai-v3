...
# 天気モジュール
from utils.weather import get_weather_for_date, display_weather
...
def create_features(df, for_prediction=False):
    ...
    features['is_rainy'] = df['weather'].isin(['雨', '小雨', '雪', '小雪']).astype(int)
...
def train_model():
    ...
    df = df.rename(columns={
        ...
        '間隔': 'rest_weeks', '馬場状態': 'track_condition', '天気': 'weather'
    })
    ...
    print(f"  → 特徴量重要度Top5: {', '.join(importance.head(5)['feature'].tolist())}")
    
    return model

def load_racecard(date: str, racecard_path: Path):
...
def predict_races(model, date: str, racecard_path: Path, odds_path: Path):
    print(f"\n🏇 レース予測中... ({date})")
    ...
    racecard = load_racecard(date, racecard_path)
    ...
    # 天気・馬場を設定
    df['weather'] = df['place_code'].map(lambda x: weather_data.get(x, {}).get('weather', '晴'))
    df['track_condition'] = df['place_code'].map(lambda x: weather_data.get(x, {}).get('track_condition', '良'))
...
def display_predictions(df):
    ...
    for race_id in sorted(df['race_id'].unique()):
        ...
        weather = sorted_df['weather'].iloc[0]
        track_cond = sorted_df['track_condition'].iloc[0]
        
        print(f"\n{'='*70}")
        print(f"🏇 {place} {race_no}R {race_name} ({track}{dist}m) 【{weather}/{track_cond}】")
        print(f"{'='*70}")
...
def main():
    ...
    parser.add_argument('--date', required=True, help='予測日 (YYYY-MM-DD)')
    args = parser.parse_args()
    
    date = args.date
    date_short = date.replace('-', '')[2:]  # 2025-12-14 -> 251214
    
    racecard_path = Path(f'data/csv_imports/racecard/DE{date_short}.CSV')
    odds_path = Path(f'data/csv_imports/odds/OD{date_short}.CSV')
    
    if not racecard_path.exists():
        print(f"❌ 出馬表がありません: {racecard_path}")
        return
    if not odds_path.exists():
        print(f"❌ オッズがありません: {odds_path}")
        return
    
    model = train_model()
    df = predict_races(model, date, racecard_path, odds_path)
    display_predictions(df)
    
    output_path = Path(f'temp/predictions_{date.replace("-", "")}_v3.csv')
